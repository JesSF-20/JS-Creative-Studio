...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (track && carouselContainer) {
        ...
        const updateCurve = () => {
            ...
            track.querySelectorAll('.carousel-item').forEach(item => {
                ...
                const distFromCenter = (itemCenterX - centerX) / (containerRect.width / 2);
    ...
    const openServiceModal = (title) => {
        ...
        serviceBackdrop.querySelector('.service-modal-icon').className = `service-modal-icon ${data.icon}`;
        serviceBackdrop.querySelector('.service-modal-title').textContent = title;
        serviceBackdrop.querySelector('.service-modal-text').textContent = data.text;
    ...
    window.abrirProyecto = function(src) {
        ...
        const isYouTube = cleanSrc.includes('youtube.com') || cleanSrc.includes('embed') || cleanSrc.includes('youtu.be');
